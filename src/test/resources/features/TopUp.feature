Feature: TopUp Account
  ...
  Scenario: Add money to Revolut account using debit card
    ...
    And Danny has 150.0 euro in this DebitCard
    When Danny tops up
  ...
  Scenario: Add money to Revolut account using bank account
    ...
    And Danny has 250.0 euro in this DebitCard
    When Danny tops up
  ...
  Scenario Outline: Add various amounts to Revolut account
    ...
    And Danny selects his DebitCard as his topUp method
    And Danny has <paymentServiceBalance> euro in this DebitCard
    When Danny tops up
    Then The new balance of his euro account should now be <newBalance>
    Examples:
      | startBalance| topUpAmount | newBalance  |paymentServiceBalance |
      | 0           | 100         | 100         |200                   |
      | 14          | 20          | 34          |50                    |
      | 23          | 30          | 53          |40                    |
  ...
  Rule: If money is sent to a friends account it should be removed from the users account and added to the friends account.
    ...
    Scenario: Money is added to a friends account.
      ...
      And Mary has a starting balance of 20.0
      When Danny sends Mary 20.0 euros
      Then The balance in Marys euro account should be 40.0

    Scenario: Money is removed from senders account.
      Given a person named Mary
      And Danny's starting balance is 40.0
      When Danny sends Mary 20.0 euros
      Then The balance in Marys euro account should be 20.0
